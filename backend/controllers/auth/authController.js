...
// 🛡️ IMPORT HÀM ĐỂ FIX LỖI "createAdminNotification is not defined"
// Chú ý: Bạn hãy kiểm tra lại đường dẫn '../notificationController' xem đã đúng với cấu trúc thư mục của bạn chưa nhé!
let createAdminNotification = null;
try {
  const notificationService = require('../notificationController'); // Hoặc đường dẫn chuẩn của dự án bạn
  createAdminNotification = notificationService.createAdminNotification;
} catch (e) {
  // Hàm fallback dự phòng nếu chưa có file để tránh crash server
...
exports.register = async (req, res) => {
  ...
  try {
    ...
    // Gọi hàm notification đã được import an toàn ở trên đầu file
  ...
  try {
    ...
    return res.status(200).json({
      ...
      user: {
        ...
        avatar_url: user.profile_avatar || null,
      },
    });
  } catch (error) {
...
exports.googleLogin = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      // Tạo tài khoản mới mặc định active = 1
      const [result] = await db.execute(
        `INSERT INTO Users (username, email, password, role, is_verified, is_active) VALUES (?, ?, ?, ?, ?, 1)`,
      ...
      try {
        if (role === "employer") {
          ...
          // Tạo một profile trống cho Employer
          await db.execute(
      ...
      // Khởi tạo object user hoàn chỉnh (đã dọn dẹp các dòng trùng lặp lỗi cũ)
      user = {
        id: userId,
      ...
      // Nếu user đã tồn tại, kiểm tra xem có bị Block/Ban không
      if (!user.is_active) {
        const banMsg = user.ban_reason
          ? `Tài khoản của bạn đã bị khóa. Lý do: ${user.ban_reason}`
          : "Tài khoản của bạn đã bị khóa. Vui lòng liên hệ hỗ trợ.";
