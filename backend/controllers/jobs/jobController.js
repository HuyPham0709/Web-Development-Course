  ...
  try {
    ...
    if (skills && Array.isArray(skills) && skills.length > 0) {
      ...
      if (skillRows.length > 0) {
        const insertValues = [];
        const valuePlaceholders = skillRows
          .map((row) => {
...
exports.updateJob = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Job updated successfully! The job is pending review.",
      });
  } catch (error) {
...
exports.getSuggestions = async (req, res, next) => {
  try {
    ...
    console.error(
      "Error retrieving Autocomplete data suggests the following task:",
      error,
    );
    return res
      .status(500)
