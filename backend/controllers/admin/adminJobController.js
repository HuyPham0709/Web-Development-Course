...
// 2. Xóa job (soft delete) - ĐÃ SỬA: Thêm dấu phẩy hợp lệ trước mảng [job_id]
...
// 7. Xóa nhiều Job hàng loạt (Bulk Delete)
exports.bulkDeletejobs = async (req, res) => {
    ...
    try {
        const [result] = await db.execute(
            `UPDATE jobs SET deleted_at = NOW() WHERE id IN (${ids.map(() => '?').join(',')}) AND deleted_at IS NULL`,
            ids
        );
        res.status(200).json({
            success: true,
...
// 8. Nhân bản tin tuyển dụng (Duplicate)
...
// 9. Xuất danh sách CSV lọc theo điều kiện (Export CSV)
...
// 10. Lấy chi tiết Job bằng ID (Dành cho trang View Details)
...
// 11. Cập nhật chi tiết Job (Dành cho trang lưu Edit Job)
